...
RUN opm render registry.ci.openshift.org/ocp/vcf-migration-op-bundle:main  --output=yaml >> /tmp/index.yaml
...
COPY --from=builder /tmp/index.yaml /configs/vcf-migration-operator/index.yaml
